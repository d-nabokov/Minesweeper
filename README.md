...
Эта часть алгоритма вступает в силу тогда, когда у нас нет стопроцентного решения, и приходится угадывать. Но раз наша цель выиграть,
...
1. Заводим пустую хэш-таблицу, которая будет связывать позицию ячейки и вероятность, что в ней мина.
2. Пробегаемся по списку групп. Для группы посчитаем вероятность нахождения мины вокруг неё, она будет равно весу, делённому на количество
неоткрытых ячеек. Для каждой ячейки в группе добавляем её в хэш-таблицу. Причём делаем это так: если такой ячейки ещё нет, но просто добавляем
ячейку со значением вероятности группы; иначе изменим значение вероятности в хэш-таблице, оно станет `1 - (1 - p1)(1 - p2)`. То есть на этом этапе
мы считаем, что вероятности для каждой группы независимы. Заметим, что если ячейка входит в первые k групп (без потери общности),
то её вероятность в хэш-таблице будет `1 - (1 - p1)(1 - p2)...(1 - pk)`
...
корректируем значение в хэш-таблице. Причём коррекция проводится до тех пор, пока во всех группах не равенство не будет выполнено (с заранее
заданной точностью)

Нужно сказать, что этот алгоритм даёт приближенные результаты. В этом можно убедиться на следующем примере:
...
![all_variants](docs/assets/all.png)
...
![variants_a](docs/assets/A.png)

Аналогично для B и C
![variants_b](docs/assets/B.png)
![variants_C](docs/assets/C.png)
